...
public:
    ...
    void lock() {
        cout << "Animalul este prins in capcana!" << "\n";
    };

    void unlock() {
        cout << "Animalul a scapat din capcana!" << "\n";
    };
...
class Catcher {
public:
    explicit Catcher(Animal a) : a(a) {
        a.lock();
    } // when we create the object we want to lock it so we do that in the constructor
    ~Catcher() {
        a.unlock();
    } // when we're done with the object we want to unlock so we do that in the destructor
private:
    Animal a;
};

void f() {
    shared_ptr<Animal> animal_res(new Animal("melc", "terestru", 0));   //the shared_ptr smart pointer is used for managing resources of object animal_res,
                                                                        //the object will be deleted and resources released before control leaves function f
}
